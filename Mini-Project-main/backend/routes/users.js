...
  try {
    // Use req.user.id for consistency if the auth middleware sets `id` or use `_id`
...
  try {
    // Note: Allowing Aadhaar number update here might require extra verification steps in a real system.
...
    // Merge preferences to avoid overwriting the entire object
...
  try {
    // NOTE: This endpoint requires integration with a file storage service (e.g., S3, Google Cloud Storage)
    // and a middleware like 'multer' for handling multipart form data.
    res.json({
      success: true,
...
    // Get recent applications and grievances (limit to max 5 for efficiency before combining)
...
      success: true,
      data: {
        // Apply pagination slice to the combined/sorted array
...
    // Select '+password' ensures the hashed password field is returned from the database
...
    // Verify current password (assumes comparePassword method on Mongoose model)
...
    // Update password (assumes Mongoose middleware will hash the new password on save)
...
    // Initialize preferences/notifications if they don't exist
...
  try {
    // Exclude sensitive fields for admin/staff view
